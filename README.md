...
### Directions
In the queries.js file write the following:
Run ```node queries.js``` to test your queries. 

1. write a function that fetches all `people` from the db and returns the array
1. fetch all the `people` from the db, but only return their `name` and `company` using the `forEach()` array method
1. fetch only people who have id's greater than 10
1. Create two new people and make sure they are saved to the db.  How might you verify this both in psql and using sequelize?
1. Fetch one of the people you just created and update/save their role
1. Fetch another person you did not create and update their name to your own
1. Create yet another person, verify that they are in the db, then fetch and destroy that person
